...
public:
  ...
  std::vector<Edge> const &neighbours(size_t node) const
  {
    return edges[node];
  }
...
int main()
{
  ...
  for (size_t y = 0; y != N; ++y)
  {
    for (size_t x = 1; x != N; ++x)
    {
      ...
      graph.add_edge(id(x, y, N), id(x - 1, y, N), square[y][x - 1]);
    }
  }
  ...
  // Start
  graph.add_edge(start, id(0, 0, N), square[0][0]);

  std::cout << shortest_path(graph, start, N * N - 1) << '\n';
}